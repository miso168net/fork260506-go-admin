...
> **Verification note (T010)**: Re-verified at HEAD `9ad098f` on 2026-05-07 against
> the current branch tip. Drift was found in entries 3, 5, 6, 7, and 8; the table
> below has been corrected to reflect the actual lines. Entries 1, 2, 4, 9, 10,
> and 11 were confirmed unchanged. (See `auth.go:49–62` swagger range — also
> corrected previously in T008 fixup.)
...
| 3 | Authenticator | `common/middleware/handler/auth.go:63–107` | `func Authenticator(c *gin.Context) (interface{}, error)` | T010 drift fix: symbol is exported (capital A); function spans lines 63–107 (was cited as `~60–105` — corrected). Reads request body into `Login`, calls `captcha.Verify`, calls `Login.GetUser`, calls `LoginLogToDB` on both success and failure paths. |
| 4 | Captcha verify | `common/middleware/handler/auth.go:88` | `captcha.Verify(loginVals.UUID, loginVals.Code, true)` | The `true` flag = **clear-after-verify** (one-time use). Dev-mode bypass is documented in the swagger comment block at `auth.go:49–62` (T008 fixup). |
| 5 | Login struct + GORM user lookup | `common/middleware/handler/login.go:9–14` (struct), `:16–33` (`GetUser` method) | `Login{Username, Password, Code, UUID}` + `GetUser(tx *gorm.DB)` | T010 drift fix: struct ends at line 14 and `GetUser` starts at line 16, ending at line 33 (file is 33 lines total — was cited as `9–17` / `17–39`). First query: `SELECT * FROM sys_user WHERE username = ? AND status = '2'`. Status `'2'` means *active* (NOT a typo for boolean). |
| 6 | Bcrypt password compare | `common/middleware/handler/login.go:22` | `pkg.CompareHashAndPassword(user.Password, u.Password)` | T010 drift fix: actual line 22 (was cited as 24). bcrypt internals are out of scope (see FR-016 + spec Out-of-scope). |
| 7 | GORM role lookup | `common/middleware/handler/login.go:27` | `tx.Table("sys_role").Where("role_id = ? ", user.RoleId).First(&role).Error` | T010 drift fix: actual line 27 (was cited as 30). Second query: `SELECT * FROM sys_role WHERE role_id = ?`. |
| 8 | PayloadFunc (build claims) | `common/middleware/handler/auth.go:21–35` | `PayloadFunc(data interface{}) jwt.MapClaims` | T010 drift fix: function body closes at line 35 (was cited as 21–37, which extended past the closing brace). Claims: `identity`, `roleid`, `rolekey`, `nice`, `datascope`, `rolename`. `IdentityKey` etc. are constants from gin-jwt SDK. |
...
| 11 | Response assembly | `common/middleware/handler/auth.go:155` (`LogOut` 2-key `{code,msg}`), `:178` (`Unauthorized` 2-key `{code,msg}`); login-success shape comes from `fork260506-go-admin-core/sdk/pkg/jwtauth/`'s default `LoginResponse` callback (this fork does NOT override it). | `c.JSON(http.StatusOK, gin.H{...})` in LogOut/Unauthorized; SDK boundary for login success. | **T012 finding (was wrongly framed in earlier draft)**: only `LogOut` and `Unauthorized` `c.JSON` calls live in `auth.go`. The 5-key login-success response (`code/currentAuthority/expire/success/token` per §3.2) is produced by gin-jwt SDK's default `LoginResponse`. Doc layer 5.11 honestly discloses this and treats the SDK shape as the FR-010 SDK boundary. |
